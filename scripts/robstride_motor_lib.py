"""
RobStride Motor Communication Library for CAN Socket (MIT Control Mode).
Supports RobStride RS00, RS01, RS02, RS03, RS04 actuators.
"""
...
class RobStrideMotorController:
    """RobStride RS-series CAN bus motor controller supporting MIT impedance control."""
    ...
    COMMAND_MODES = {
        "GET_DEVICE_ID": 0,
        "MOTOR_CONTROL": 1,        # MIT Impedance Control
        "MOTOR_FEEDBACK": 2,
        "MOTOR_ENABLE": 3,
        "MOTOR_STOP": 4,
    ...
    RUN_MODES = {
        "CONTROL_MODE": 0,         # MIT Mode (p, v, kp, kd, tau_ff)
        "POSITION_MODE": 1,
        "VELOCITY_MODE": 2,
        "CURRENT_MODE": 3,
    }
    ...
    def __init__(self, bus="can0", motor_id=127, main_can_id=254, motor_dir=1, motor_type="RobStride00"):
        ...
        if motor_type == "RobStride00":
            self.motorParams = copy.deepcopy(RobStride00_PARAMS)
        elif motor_type in ("RobStride01", "RobStride02"):
        ...
        if bus not in RobStrideMotorController.bus:
            try:
                RobStrideMotorController.bus[bus] = can.interface.Bus(
                    interface="socketcan", channel=bus, bitrate=1000000
                )
            except Exception as e:
                print(f"[RobStride] SocketCAN Error on {bus}: {e}")
    ...
    def set_angle_range(self, low, upper, deg=False):
        if deg:
            self.angle_range = [low * math.pi / 180.0, upper * math.pi / 180.0]
        else:
    ...
    def write_single_param(self, param_name, value):
        ...
        if param_info is None:
            print(f"[RobStride] Unknown parameter name: {param_name}")
            return
    ...
    def set_run_mode(self, mode):
        if mode not in self.RUN_MODES:
            raise ValueError(f"Invalid mode: {mode}. Must be one of {list(self.RUN_MODES.keys())}")
...
CanMotorController = RobStrideMotorController
